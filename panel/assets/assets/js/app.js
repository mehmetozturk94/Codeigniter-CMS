+(function ($, window) {
  ...
  app.$navbar
    .removeClass("primary")
    .addClass(app.settings.navbar.theme)
    .addClass("in");
  app.$body
    .removeClass("theme-primary")
    .addClass("theme-" + app.settings.navbar.theme);

  // initialize menubar
  app.$menubar
    .removeClass("light")
    .addClass(app.settings.menubar.theme)
    .addClass("in");
  app.$body
    .removeClass("menubar-light")
    .addClass("menubar-" + app.settings.menubar.theme);
  ...
  var menubar = {
    ...
    listenForEvents: function () {
      ...
      $(window).on("load", function (e) {
        ...
        if (app.topbarLayout && current.name !== "xs")
          $(document).on(
            ...
            self.highlightOpenLink.bind(self)
          );
        else self.highlightOpenLink();
        ...
        if (app.defaultLayout && current.name !== "xs" && current.name !== "lg")
          self.fold();
      });
      ...
      $(document).on(
        ...
        self.toggleTopbarSubmneuOnHover
      );
    ...
    reduceAppMenu: function () {
      ...
      if ($menuItems.length > 5) {
        var $moreItemsLi = $('<li class="more-items-li has-submenu"></li>'),
          ...
          $moreItemsToggle = $(
            '<a href="javascript:void(0)" class="submenu-toggle"></a>'
          );
        $moreItemsToggle.append([
        ...
        $moreItemsLi
          .append([$moreItemsToggle, $moreItemsUl])
          .insertAfter($appMenu.find(">li:nth-child(5)"));
      }
    ...
    toggleTopbarSubmneuOnHover: function (e) {
      var $this = $(this),
        total = $this.offset().left + $this.width();
      var ww = $(window).width();
      if (ww - total < 220) {
    ...
    fold: function () {
      ...
      $appMenu.find("li.open").removeClass("open") &&
        $appMenu.find(".submenu").slideUp();
      return true;
    },
    ...
    pullIn: function () {
      $body.addClass("menubar-in") &&
        $menubarToggleButton.addClass("is-active") &&
        (this.open = true);
      return true;
    },
    ...
    pushOut: function () {
      $body.removeClass("menubar-in") &&
        $menubarToggleButton.removeClass("is-active") &&
        (this.open = false);
      return true;
    },
    ...
    initScroll: function () {
      var $scrollInner = $(
        "body.menubar-left.menubar-unfold .menubar-scroll-inner"
      );
    ...
    highlightOpenLink: function () {
      var currentPageName = location.pathname.slice(
          location.pathname.lastIndexOf("/") + 1
        ),
        currentPageLink = $appMenu
          .find('a[href="' + currentPageName + '"]')
          .first();
    ...
    getAppliedTheme: function () {
      var appliedTheme = "",
        themes = app.settings.menubar.themes,
        theme;
      for (theme in themes) {
  ...
  var navbar = {
    ...
    listenForEvents: function () {
      $(document).on("click", '[data-toggle="collapse"]', function (e) {
        ...
        $trigger.is('[data-toggle="collapse"]') ||
        ...
        if ($target.attr("id") === "navbar-search") {
          if (!$trigger.hasClass("collapsed")) {
            ...
            document
              ...
              .setSelectionRange(0, $field.val().length);
          } else {
    ...
    applyTheme: function () {
      ...
      $body
        .removeClass("theme-" + appliedTheme)
  ...
  var customizer = {
    ...
    initCustomizer: function () {
      ...
      app.settings.menubar.folded &&
        $("#menubar-fold-switch").prop("checked", true);
    },
    ...
    getTemplate: function (themeName) {
      var html =
        '<div class="theme-choice radio radio-' + themeName + ' m-b-md">';
      html +=
